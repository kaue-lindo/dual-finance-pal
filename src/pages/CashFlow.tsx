...
const CashFlow = () => {
  ...
  const prepareChartData = () => {
    ...
    transactions.forEach(transaction => {
      ...
      // Verificar se o mês está dentro do período de 12 meses que estamos mostrando
      ...
      if (transaction.type === 'income') {
        if (transaction.category === 'investment_returns' || transaction.category === 'investment-return') {
          // Rendimentos de investimentos agora são considerados parte dos investimentos
        ...
        if (transaction.category === 'investment_update') {
          // Reinvestimentos são tratados como parte dos investimentos
          monthlyData[monthKey].investment -= transaction.amount; // Negativo porque está aumentando o valor do investimento
        } else {
    ...
    Object.keys(monthlyData).sort((a, b) => {
      ...
      if (monthKey === format(today, 'MMM/yy')) {
        ...
        // Investimentos afetam o saldo (saída de dinheiro)
        runningBalance -= monthlyData[monthKey].investment;
        
        // Rendimentos de investimentos são reinvestidos (não afetam o saldo)
